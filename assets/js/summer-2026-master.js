(() => {
  const VERSION = '20260818realcars12';
  ...
  function injectTransport() {
    ...
    const cars=[
      {photo:'https://upload.wikimedia.org/wikipedia/commons/thumb/0/07/Chevrolet_Suburban_LT_%282021%29_%2851299181015%29.jpg/960px-Chevrolet_Suburban_LT_%282021%29_%2851299181015%29.jpg',fallback:'',name:'Chevrolet Suburban',credit:'Photo: Charles · CC BY 2.0',place:'Seattle + Canadian Rockies',note:'הרכב הגדול של פרק הכבישים הארוך דרך קנדה והרוקייז.'},
      {photo:'https://upload.wikimedia.org/wikipedia/commons/thumb/9/9c/2022_Nissan_Pathfinder_SV_in_Super_Black%2C_Front_Left%2C_03-22-2022.jpg/960px-2022_Nissan_Pathfinder_SV_in_Super_Black%2C_Front_Left%2C_03-22-2022.jpg',fallback:'',name:'Nissan Pathfinder',credit:'Photo: Elise240SX · CC BY-SA 4.0',place:'Big Island',note:'הרכב של ביג איילנד, מקונה ועד הנסיעות הארוכות ברחבי האי.'},
      {photo:'https://upload.wikimedia.org/wikipedia/commons/thumb/7/7d/%2721_Chevrolet_Tahoe.jpg/960px-%2721_Chevrolet_Tahoe.jpg',fallback:'',name:'Chevrolet Tahoe',credit:'Photo: Bull-Doser · Public domain',place:'Maui',note:'הרכב של מאווי, כולל Road to Hana והנסיעות ברחבי האי.'},
      {photo:'https://upload.wikimedia.org/wikipedia/commons/thumb/7/7b/2024_Chevrolet_Traverse_front_view.jpg/960px-2024_Chevrolet_Traverse_front_view.jpg',fallback:'',name:'Chevrolet Traverse',credit:'Photo: Deathpallie325 · CC BY-SA 4.0',place:'Los Angeles',note:'הרכב של הפרק האחרון בלוס אנג׳לס לפני הדרך הביתה.'}
    ];
    const section=document.createElement('section');
    section.className='transportMaster';
    section.innerHTML=`<div class="wrap"><div class="sectionhead"><h2>באוויר ועל הכביש</h2><p>שש טיסות, ארבע חברות תעופה וארבעה רכבים. הדרך עצמה היא חלק מהסיפור.</p></div><div class="flightJourney"><div class="flightJourneyTop"><div><small>FLIGHT JOURNAL</small><h3>שש טיסות.<br>ארבע חברות.</h3></div><p>רצף הטיסות המלא, מהיציאה מישראל ועד החזרה הביתה.</p></div><div class="flightLegs">${legs.map(([a,from,to,note])=>`<article class="flightLeg"><div class="airBrand"><img src="${airlines[a][0]}" alt="${airlines[a][1]}"><strong>${airlines[a][1]}</strong></div><div class="flightRoute">${from} → ${to}</div><span>${note}</span></article>`).join('')}</div></div><div class="carsMaster"><div class="carsMasterHead"><h3>ארבעה רכבים. ארבעה פרקים.</h3><p>כל רכב מסמן שינוי אחר בקצב ובאופי של הדרך.</p></div><div class="carGridMaster">${cars.map((c,i)=>`<article class="carMaster"><div class="carMasterMedia"><img src="${c.photo}" data-fallback="${c.fallback}" alt="${c.name}" loading="lazy"></div><div class="carMasterBody"><small>0${i+1}</small><h4>${c.name}</h4><p><strong>${c.place}</strong><br>${c.note}</p><p class="carMasterCredit">${c.credit || ""}</p></div></article>`).join('')}</div></div></div>`;
